...
class FailFastErrorHandler {
    ...
    closed() {
        vscode.window.showErrorMessage("The SC language server has crashed");
...
function activate(context) {
    var config = vscode.workspace.getConfiguration("sc");
        
    var binaryPath = config.get("languageServerPath");
    if (!binaryPath) {
        vscode.window.showErrorMessage("Could not start SC language server due to missing setting: sc.languageServerPath");
        return;
    }

    var args = config.get("languageServerArguments");
    if (!args) {
        vscode.window.showErrorMessage("Could not start SC language server due to missing setting: sc.languageServerArguments");
        return;
    }
    ...
    // Test Current File
    context.subscriptions.push(vscode.commands.registerCommand('sc.runTestsInCurrentFile', function(args) {
    ...
    console.log("Running SC Language server...");

    var client = new vscode_languageclient.LanguageClient("scLanguageServer", "SC language server", serverOptions, clientOptions);
    client.start(); 
}
...
function getOutputChannel() {
    if (!_channel) {
        _channel = vscode.window.createOutputChannel('SC Test');
    }
    return _channel;
}
