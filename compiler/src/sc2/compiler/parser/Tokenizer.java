...
public class Tokenizer {
  ...
  private boolean isDoc;        // return documentation comments or if false ignore them
  ...
  public Tokenizer(CompilerLog log, Loc loc, String buf)
  {
    this.log = log;
    this.buf      = buf;
    ...
    cur = peek = ' ';
    if (buf.length() > 0) cur  = buf.charAt(0);
    if (buf.length() > 1) peek = buf.charAt(1);
    pos = 0;
  ...
  **/
  public ArrayList<Token> tokenize()
  {
    while (true)
    {
      Token tok = next();
      tokens.add(tok);
      if (tok.kind.equals(TokenKind.eof)) break;
    }
    return tokens;
  }
...
//    return null; // TODO - shouldn't need this
  ...
  **/
  ...
  {
    // skip whitespace
    if (Character.isSpaceChar(cur)) { consume(); whitespace = true; return null; }
    ...
    if (cur == '.' && Character.isDigit(peek)) return number();

    // str literal
    if (cur == '"' && peek == '"' && peekPeek() == '"') return quoted(true);
    if (cur == '"') return quoted(false);
    if (cur == '\'') return ch();
...
//////////////////////////////////////////////////////////////////////////
  ...
  **/
  private Token word()
  {
    ...
    while (Character.isAlphabetic(cur) || Character.isDigit(cur) || cur == '_' || cur > 256) consume();
...
//////////////////////////////////////////////////////////////////////////
  ...
  **/
  private Token number()
  {
    ...
    // whole part
    while (Character.isDigit(cur) || cur == '_') consume();
    ...
    if (cur == 'e' || cur == 'E')
    {
      ...
      exp = true;
      if (cur == '-' || cur == '+') consume();
      if (!Character.isDigit(cur)) throw err("Expected exponent digits");
      while (Character.isDigit(cur) || cur == '_') consume();
    }
  ...
  **/
  Token hex()
  {
    ...
    if (val == -1) throw err("Expecting hex number");
    consume();
    int nibCount = 1;
    while (true)
    {
    ...
    for (int i=openLine; i<openPos; ++i)
    {
      ...
      if ((a == '\t' && cur != '\t') || (a != '\t' && cur != ' '))
      {
        ...
        for (int j=openLine; j<openPos; ++j)
          { if (buf.charAt(j) == '\t') ++numTabs; else ++numSpaces; }
        if (numTabs == 0)
  ...
  private Token symbol()
  {
    ...
    switch (c)
    {
      ...
      case '!':
        ...
        {
          consume();
          if (cur == '=') { consume(); return new Token(TokenKind.notSame); }
      ...
      case '%':
        if (cur == '=') { consume(); return new Token(TokenKind.assignPercent); }
        return new Token(TokenKind.percent);
      case '&':
        if (cur == '&') { consume(); return new Token(TokenKind.doubleAmp); }
      ...
      case '*':
        if (cur == '=') { consume(); return new Token(TokenKind.assignStar); }
        return new Token(TokenKind.star);
      case '+':
        if (cur == '=') { consume(); return new Token(TokenKind.assignPlus); }
        if (cur == '+') { consume(); return new Token(TokenKind.increment); }
        return new Token(TokenKind.plus);
      case ',':
        return new Token(TokenKind.comma);
      case '-':
        if (cur == '>') { consume(); return new Token(TokenKind.arrow); }
        if (cur == '-') { consume(); return new Token(TokenKind.decrement); }
        if (cur == '=') { consume(); return new Token(TokenKind.assignMinus); }
        return new Token(TokenKind.minus);
      case '.':
        if (cur == '.')
        {
          consume();
          if (cur == '.') { consume(); return new Token(TokenKind.dotDotDot); }
          if (cur == '<') { consume(); return new Token(TokenKind.dotDotLt); }
          return new Token(TokenKind.dotDot);
        }
        return new Token(TokenKind.dot);
      case '/':
        if (cur == '=') { consume(); return new Token(TokenKind.assignSlash); }
        return new Token(TokenKind.slash);
      case ':':
        if (cur == ':') { consume(); return new Token(TokenKind.doubleColon); }
        if (cur == '=') { consume(); return new Token(TokenKind.defAssign); }
      ...
      case '<':
        ...
        {
          consume();
          if (cur == '>') { consume(); return new Token(TokenKind.cmp); }
      ...
      case '=':
        ...
        {
          consume();
          if (cur == '=') { consume(); return new Token(TokenKind.same); }
          return new Token(TokenKind.eq);
        }
        return new Token(TokenKind.assign);
      case '>':
        if (cur == '=') { consume(); return new Token(TokenKind.gtEq); }
        return new Token(TokenKind.gt);
      case '?':
        if (cur == ':') { consume(); return new Token(TokenKind.elvis); }
        if (cur == '.') { consume(); return new Token(TokenKind.safeDot); }
        if (cur == '-' && peek == '>') { consume(); consume(); return new Token(TokenKind.safeArrow); }
        if (cur == '~' && peek == '>') { consume(); consume(); return new Token(TokenKind.safeTildeArrow); }
      ...
      case '|':
        if (cur == '|') { consume(); return new Token(TokenKind.doublePipe); }
        return new Token(TokenKind.pipe);
      case '}':
        return new Token(TokenKind.rbrace);
      case '~':
        if (cur == '>') { consume(); return new Token(TokenKind.tildeArrow); }
