  ...
  **
  Void parse()
  {
    ...
    while (curt !== Token.eof) {
      try {
        defNode := topLevelDef
        if (defNode != null) unit.addDef(defNode)
      }
      catch (CompilerErr e) {
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  ...
  private DefNode? topLevelDef()
  {
    ...
    if (curt === Token.mixinKeyword || curt === Token.enumKeyword || curt === Token.structKeyword) {
        return typeDef(doc, facets, flags)
    }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  TypeDef typeDef(DocDef? doc, FacetDef? facets, Int flags)
  {
...
//        }
        param := GeneriParamDefDef(gLoc, paramName, def, gparams.size)
        gparams.add(param)
        if (curt === Token.comma) {
          consume
          continue
      ...
      def.generiParamDefeters = gparams
    }
...
//      return ResolveImports.resolveQualified(this, id, consumeId, loc) ?: ns.voidType
      type = TypeRef.makeRef(loc, id, consumeId)
    }
    else {
      type = TypeRef.makeRef(loc, null, id)
    }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  protected Void endLoc(Node node) {
