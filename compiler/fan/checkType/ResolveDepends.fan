...
**
** ResolveDepends resolves each dependency to a PodDef and
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  override Void run()
  {
    ...
    if (pod.depends.isEmpty) {
      if (!isSys) {
        pod.depends.add(Depend.fromStr("sys 1"))
      }
    }

    // we initialize the CNamespace.depends map
    // as we process each dependency
    compiler.pod.resolvedDepends = [Str:PodDef][:]
  ...
  **
  private PodDef? resolveDepend(Depend depend)
  {
    PodDef? pod
    try
    {
