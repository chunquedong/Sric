...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  ** Parse the signature into a resolved TypeRef.  We *don't*
  ** use the CNamespace's cache - it is using me when a signature
  ** isn't found in the cache.  But we do use the PodDef's type cache
  ** via PodDef.resolveType.
  **
  public static TypeRef parse(Str sig)
  {
...
//      }
      ...
      return TypeRef.make(podName, typeName)
...
//      t = t.toListOf
  ...
  private TypeRef loadGeneriParamDef(TypeRef base) {
    consume('<')
    params := TypeRef[,]
    while (true) {
      if (cur == '>') {
...
//    return FuncType(Loc.makeUnknow params, names, ret)
  ...
  private TypeRef loadFFI()
  {
    ...
    return TypeRef.makeQname(qname);
  }

  private TypeRef loadBasic()
  {
    start := pos
    while (cur != ':' || peek != ':') consume
    consume(':')
