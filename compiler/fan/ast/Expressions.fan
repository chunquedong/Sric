...
class FieldExpr : NameExpr
{
  ...
  new makeField(Loc loc, Expr? target, FieldDef field, Bool useAccessor := true)
...
//    this.ctype = ctype
  ...
  new makeType(Loc loc, TypeRef ctype)
...
class TypeCheckExpr : Expr
{
  ...
  {
    //if (to.isGeneriParamDefeter) to = to.raw
    this.target = target
  ...
  ** From type if coerce
  TypeRef? from { get { &from ?: target.ctype } }
