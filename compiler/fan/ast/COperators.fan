...
class COperators
{
  ...
  new make(TypeDef parent)
  {
    ...
    parent.slots.each |slot|
    {
      if (slot is MethodDef && slot.isOperator)
      {
        ...
        {
          acc := byPrefix[prefix]
          if (acc == null) byPrefix[prefix] = acc = MethodDef[,]
          acc.add(slot)
        }
      }
    }
  ...
  **
  MethodDef[] find(Str prefix)
  {
    byPrefix[prefix] ?: MethodDef#.emptyList
  }
  ...
  TypeDef parent { private set }
  private Str:MethodDef[] byPrefix := [:]
}